...
- ✅ **多轮对话记忆**：保留最近 N 轮问答上下文，支持指代追问（如「他出什么装备」）；检索与 LLM 生成均结合历史
...
用户游戏问答输入
        ↓
读取多轮对话记忆（近期 user/assistant 轮次）
        ↓
拼接上下文查询（用于意图识别 + RAG/MCP 检索）
        ↓
规则意图路由（knowledge / time / wiki）
├─ 静态游戏知识查询 → BM25+向量混合检索 + document_id 联动 MD 原文
├─ 游戏时间/倒计时/对局时序需求 → 调用时间计算 MCP → 输出精准时间推算结果
└─ 实时资讯/版本改动/外网打法需求 → RAG + 百科抓取 MCP → 补充全网实时内容
        ↓
system + 历史消息 + 当前轮（含参考资料）→ LLM 生成
        ↓
写入对话记忆，统一格式化回复用户
```
...
助手在单次 `python main.py` 会话内维护对话状态，无需额外数据库。

| 环节 | 策略 |
|------|------|
| **记忆存储** | `agent/memory.py` 保存用户原问题与助手回复，默认保留最近 10 轮 |
| **LLM 生成** | 将历史消息一并送入模型，理解「他」「这个」「还有呢」等指代 |
| **检索 / MCP** | 用「近期对话 + 当前问题」作为查询，避免省略主语导致 RAG 搜偏 |
| **当前轮资料** | RAG/MCP 结果仅附加在本轮 user 消息，不重复写入历史，控制 token |
| **清空记忆** | 输入 `clear`、`清空`、`新对话` 等命令可开始新话题 |

环境变量 `MEMORY_MAX_TURNS`（默认 `10`）控制保留轮数，见 `.env.example`。
...
│   └── chunking.py      # 文本分块
├── agent/
│   ├── assistant.py     # 助手主逻辑（RAG/MCP + LLM）
│   ├── memory.py        # 多轮对话记忆
│   └── router.py        # 意图路由
...
- `MEMORY_MAX_TURNS`：多轮对话保留轮数（默认 `10`）
...
启动后支持连续追问；输入 `quit` 退出，输入 `clear` 或 `清空` 重置对话记忆。
